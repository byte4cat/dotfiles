...
# --- 預檢 (i3/X11 環境) ---
command -v rofi >/dev/null 2>&1 || { echo "Error: Rofi not found."; exit 1; }
command -v xdotool >/dev/null 2>&1 || { echo "Error: xdotool not found."; exit 1; }
command -v xclip >/dev/null 2>&1 || { echo "Warning: xclip not found. Copy to clipboard will be disabled."; }


# 1. 使用 sed 讀取腳本底部的數據行
# 2. 透過 rofi 管道 (dmenu 模式) 進行選擇
# 3. 截斷字串，只取第一個 emoji 字符
# 4. 移除換行符
EMOJI="$(sed '1,/^### DATA ###$/d' "$0" | rofi -p "Emoji 🔍" -dmenu -i | cut -d ' ' -f 1 | tr -d '\n')"


# 如果使用者按 Esc 取消，EMOJI 會是空值，此時退出
if [ -z "$EMOJI" ]; then
    exit 0
fi
...
# 1. 模擬鍵盤輸入 (打出 emoji)
# 注意：這可能需要你的系統正確配置了輸入法
xdotool type "$EMOJI"

# 2. 複製到 X11 剪貼簿 (如果 xclip 存在)
if command -v xclip >/dev/null 2>&1; then
    # -selection clipboard: 複製到全局剪貼簿 (Ctrl+C)
    # -selection primary: 複製到主要選區 (鼠標選中後，中鍵貼上)
